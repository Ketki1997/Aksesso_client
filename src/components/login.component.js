    ...
    if (this.checkBtn.context._errors.length === 0) {
      AuthService.login(this.state.username, this.state.password).then(
        () => {
          ...
          var verifyuser=JSON.parse(userdata);
          console.log(verifyuser.JwtToken);
          var data=localStorage.getItem("user");
          var token=JSON.parse(data);
         
          if(verifyuser.Email==='admin@gmail.com')
          {
            if(token.JwtToken===verifyuser.JwtToken){
               this.props.history.push("/HeaderComponent");
          window.location.reload();
              alert("admin dahboard");
            }
          
          }
          else if(verifyuser.Email!=='admin@gmail.com')
          {
            if(token.JwtToken===verifyuser.JwtToken){
            ...
            alert("Your not completed your registration");
            this.props.history.push("/register.component");
            window.location.reload();
           }
          }
        },
    ...
    return (
      ...
      <Col lg={6} md={6} sm={12} className="text-center pt-4">
        ...
        {this.state.message && (
          ...
          <small className="reset mr-3">New User ?</small>
          </Link>
            </div>
